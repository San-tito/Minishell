...
/*
 *	If last token is ')' we check the validity of the following
 	(curr_token_type). (must be |,&,||,),<,<<,>,>>.)
 *	If last token is '(' we also check the validity of the following
 	(curr_token_type). (must be <,<<,>,>>,(,str)
 ...
 */
static char	boundary_valid(char last_tkntype, t_token *token)
{
	if (last_tkntype == CLOSE_PAR_TOKEN)
	{
		if (token->type < CLOSE_PAR_TOKEN)
			return (0);
	}
	else if (last_tkntype == OPEN_PAR_TOKEN)
	{
		if (!((token->type >= INPUT_TOKEN && token->type <= APPEND_TOKEN)
				|| token->type <= OPEN_PAR_TOKEN))
			return (0);
	}
	if (token->type == CLOSE_PAR_TOKEN)
	{
		if (!(last_tkntype == CLOSE_PAR_TOKEN || last_tkntype == STR_TOKEN))
			return (0);
	}
	else if (token->type == OPEN_PAR_TOKEN)
	{
		if (!(last_tkntype == AND_TOKEN || last_tkntype == PIPE_TOKEN
				|| last_tkntype == OR_TOKEN))
			return (0);
	}
	return (1);
}
...
/*
 *	Checks that before an OPEN_PARENTHESIS_TOKEN there is always another
 	OPEN_PARENTHESIS_TOKEN
 *	or a TOKEN that is: (TOKEN != TOKEN_STR && TOKEN !=
 	CLOSE_PARENTHESIS_TOKEN).
 *	Also checks that after an OPEN_PARENTHESIS_TOKEN there is always a
 	TOKEN_STR or
 *	another OPEN_PARENTHESIS_TOKEN.
 *	Also checks that before a CLOSE_PARENTHESIS_TOKEN there is always
 	another CLOSE_PARENTHESIS_TOKEN
 *	or a TOKEN_STR.
 *	Also checks that after a CLOSE_PARENTHESIS_TOKEN there is always
 	another CLOSE_PARENTHESIS_TOKEN
